...
export function Navigation() {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (
        (!menuHambLeft.current ||
          !menuHambLeft.current.contains(event.target)) &&
        (!menuHambRight.current ||
          !menuHambRight.current.contains(event.target))
      ) {
  ...
  return (
    <header className="hidden md:flex bg-pl_green_a pt-2 pb-2 shadow items-center justify-between relative">
      ...
      {/* Navegación */}
      <nav className="hidden md:flex ml-auto mr-10 gap-8 font-baloo text-pl_white_a dark:text-pl_green_b relative">
        {[
          {
            title: "PLANTAS",
            links: [
